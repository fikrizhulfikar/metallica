...
public class Alt {
    ...
    public void route(String url){
        ...
        Route route = (req, res)->{
            AltHttpRequest altHttpRequest = null;
            AltHttpResponse altHttpResponse = new AltHttpResponse(res, instance);
            try {
                altHttpRequest = new AltHttpRequest(req, instance);
            }catch (Exception e){
                return instance.response(altHttpRequest, altHttpResponse, Alt.STATUS_ERROR, e.getMessage(), null);
            }
            ...
            try{
                ...
                if(e.getClass().equals(ClassNotFoundException.class) || e.getClass().equals(NoSuchMethodException.class)){
                    return instance.response(altHttpRequest, altHttpResponse, Alt.STATUS_NOTFOUND, "Tidak dapat terhubung ke server", null);
    ...
    public String response(AltHttpRequest altHttpRequest, AltHttpResponse altHttpResponse, int status, String message, Object res){
        ...
        altHttpResponse.header("Content-Type", "application/json");

        // set http status
        altHttpResponse.status(this.surpressResponseCode ? 200 : (Integer) output.get("s"));
        String result = "";
        ...
        try{
            ...
            return (altHttpRequest == null && Alt.instance().environment == Alt.ENV_PRODUCTION) || (altHttpRequest != null && altHttpRequest.secure && Alt.instance().environment == Alt.ENV_PRODUCTION) ? secure.encrypt(result) : result;
        }catch (Exception e){
            return result;
        }
    }
    ...
    public static boolean check(int permission, AltHttpRequest altHttpRequest){
        ...
        if(permission == 0)
            return islogin(altHttpRequest);

        if(!islogin(altHttpRequest))
            return false;

        boolean isallowed = false;
        String[] levels = userdata.get("userlevel").split(",");
        for(int i=0; i<levels.length; i++){
            isallowed = isallowed || ((Integer.valueOf(levels[i]) & permission) > 0);
        }
