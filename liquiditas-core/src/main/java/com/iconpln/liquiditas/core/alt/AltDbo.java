...
public class AltDbo {
    ...
    public String get_sql(){
        return this.get_sql(new HashMap<>());
    }
    public String get_sql(Map<String, String> data){
        return "select " + this.get_select(data) + " from " + this.get_tablename() + this.get_where(data) + this.get_group(data) + this.get_order(data) + this.get_join(data) + this.get_limit(data);
    }

    public AltDbRow retrieve(){
        return this.retrieve(new HashMap<>());
    }
    public AltDbRow retrieve(Map<String, String> data){
        if(data.get(this.pkey) != null)
            data.put(this.pkey, "= " + data.get(this.pkey));

        AltDbRows res = this.get(data);
        return res.size() > 0 ? res.get(0) : new AltDbRow();
    }
    ...
    public int insert(Map<String, String> data) throws AltException {
        ...
        if (table_fields.get("isdeleted") != null) {
        ...
        for (Map.Entry<String, String> entry : data.entrySet()) if(table_fields.get(entry.getKey()) != null) {
            ...
            // Titik jadi Koma Gan
//            values.add(entry.getValue() == null ? null : this.quote(entry.getValue().replace('.',',')));
            // Koma jadi Null Gan
//            values.add(entry.getValue() == null ? null : this.quote(entry.getValue().replace(",","")));
    ...
    public int update(Map<String, String> data) throws AltException {
        ...
        for (Map.Entry<String, String> entry : data.entrySet()) if(table_fields.get(entry.getKey()) != null) {
            ...
            // Cek Apabila Nilainya Titik Jadi Koma
//            value = value.replace('.',',');
            // Cek Apabila Nilainya Koma Jadi Null
//            value = value.replace(",","");
    ...
    public int simpan() throws AltException {
        return this.insert(new HashMap<>());
    }
    public int simpan(Map<String, String> data) throws AltException {
        // sql query
        String sql      = "insert into " + this.table_name + " (";
