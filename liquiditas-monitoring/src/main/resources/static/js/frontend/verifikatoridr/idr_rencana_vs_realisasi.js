...
function search() {
    showLoadingCss();
    var tgl_pencarian = $('#tgl_pencarian').val();
    if (tgl_pencarian == null || tgl_pencarian == '') {
        return;
    }
    $.ajax({
        url: baseUrl + "api_dashboard/idr_rencana_vs_realisasi_by_tgl?tgl_pencarian=" + tgl_pencarian,
        dataType: 'JSON',
        type: "GET",
        success: function (res) {
            var d = new Date(res.tglcetak);
