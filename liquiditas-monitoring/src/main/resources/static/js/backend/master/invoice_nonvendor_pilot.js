...
function getInvoiceNonVendorPilot(){
    if (validateForm("#form") !== 0){
        Swal.fire('Oops!',"Silahkan Lengkapi Isian",'info');
    }else{
        var stateCrf = confirm("Anda Yakin Akan Menarik Data Invoice Non Vendor? (Pastikan Data Sudah Benar)");
        if (stateCrf == true) {
            showLoadingCss();
            $.ajax({
                url: `${baseUrl}api_master/integrasi_sap/get_invoice_nonvendor_pilot`,
                dataType: 'JSON',
                type: "GET",
                data: {
