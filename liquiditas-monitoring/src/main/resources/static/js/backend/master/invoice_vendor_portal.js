...
function getInvoiceVendorPortal(){
    if (validateForm("#form") !== 0){
        Swal.fire('Oops!',"Silahkan Lengkapi Isian",'info');
    }else{
        var stateCrf = confirm("Anda Yakin Akan Menarik Data Vendor Portal ? (Pastikan Data Sudah Benar)");
        if (stateCrf == true) {
            showLoadingCss();
            $.ajax({
                url: `${baseUrl}api_master/integrasi_sap/get_invoice_vendor_portal`,
                dataType: 'JSON',
                type: "GET",
                data: {
