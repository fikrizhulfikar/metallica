...
function getInvoiceLunas(){
    if (validateForm("#form") !== 0){
        Swal.fire('Oops!',"Silahkan Lengkapi Isian",'info');
    }else{
        var stateCrf = confirm("Anda Yakin Akan Menarik Data Invoice Lunas? (Pastikan Data Sudah Benar)");
        if (stateCrf == true) {
            showLoadingCss();
            $.ajax({
                url: `${baseUrl}api_master/integrasi_sap/get_invoice_lunas_pilot`,
                dataType: 'JSON',
                type: "GET",
                data: {
