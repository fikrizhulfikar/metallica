...
function search(state) {
    if ($("#tanggal").val() == "" && state != "load") {
        alert("Mohon Lengkapi Tgl Akhir");
    } else {
        initDataTablePlacement($("#tanggal").val())
        srcTglAwal = $("#tanggal").val()
    }
}
...
function initDataTablePlacement(p_tanggal){
...
//            {width: 100, targets: 4},
        ...
        "ajax":
            {
                "url":
                    baseUrl + "api_operator/rekap_invoice_belum/new_placement_lcl_header",
                "type":
                    "GET",
                "dataType":
                    "json",
