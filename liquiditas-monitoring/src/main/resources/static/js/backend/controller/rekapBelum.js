    ...
    table_rekapitulasi = $('#table-rekapitulasi').DataTable({
            ...
            "aoColumnDefs": [
                ...
                },
                {
                    "aTargets": [3],
                    "mRender": function (data, type, full) {
                        return full.UNIT_ANGGARAN;
                    }
                },
                {
                    "aTargets": [4],
                    "mRender": function (data, type, full) {
                        return full.POS_ANGGARAN;
                    }
                },
                {
                    "aTargets": [5],
                      ...
                      /*  if(newRoleUser[0].includes("DIVKEU")){
                        ...
                        if (newRoleUser[0] == "ROLE_MS_LIKUIDITAS" || newRoleUser[0] == "ROLE_DM_LIKUIDITAS") {
                            ...
                            if (full.STATUS_TRACKING == "INPUT DATA") {
                                ...
                                if(role == "ROLE_POWER_PURCHASE_LEASING" || role == "ROLE_PRIMARY_ENERGY" || role == "ROLE_INVESTMENT_I" || role == "ROLE_INVESTMENT_II" || role == "ROLE_OPERATION" || role == "ROLE_PAYMENT" || role == "ROLE_CENTRALIZED_RECEIPT"){
                                    ret_value = ret_value +
                                        '<button style="width: 15px !important;" class="btn-duplicate-data btn-sm btn-primary" title="Duplicate Data" onclick="duplicate_data(\'' + full.ID_VALAS + '\')"><i class="fa fa-clone"></i></button>'+
                                ...
                                if(newRoleUser[0] == "ROLE_MANAGER_PRIMARY_ENERGY"||newRoleUser[0] == "ROLE_MANAGER_POWER_PURCHASE" || newRoleUser[0] == "ROLE_MANAGER_OPERATION"){
                                ...
                                if(role == "ROLE_KASIR" || role == "ROLE_KASIR_IDR" || role == "ROLE_KASIR_INVESTASI") {
                                    ret_value = ret_value +
                                        '<button style="width: 15px !important;" class="btn-update-data btn-sm btn-success" title="Upload" onclick="upload_file(\'' + full.ID_VALAS + '\')"><i class="fa fa-upload"></i></button>' ;
                                }
                                if(role == role.includes("ROLE_OSS")) {
                                    ret_value = ret_value +
                                        '<button style="width: 15px !important;" class="btn-duplicate-data btn-sm btn-primary" title="Duplicate Data" onclick="duplicate_data(\'' + full.ID_VALAS + '\')"><i class="fa fa-clone"></i></button>'+
                                        '<button style="width: 15px !important;" class="btn-update-data btn-sm btn-success" title="Upload" onclick="upload_file(\'' + full.ID_VALAS + '\')"><i class="fa fa-upload"></i></button>' ;
                                }
                                ...
                                if(full.EQ_IDR > "25000000000"){
                                    ...
                                    if(role == "ROLE_ADMIN"||role == "ROLE_KASIR" || role == "ROLE_KASIR_IDR" || role == "ROLE_KASIR_INVESTASI") {
                                        ret_value = ret_value +
                                ...
                                ret_value =
                                    '<div class="btn-group">' ;
                                if(role == "ROLE_KASIR" || role == "ROLE_KASIR_IDR" || role == "ROLE_KASIR_INVESTASI" || newRoleUser[0] == "ROLE_ADMIN") {
                                    ret_value = ret_value +
                                        '<button style="width: 15px !important;" id="option-lunas" class="btn-lunas btn-sm btn-warning" title="Lunas" onclick="upd_status_tracking(\'' + full.ID_VALAS + '\',\'' + 7 + '\',\'' + full.ID_JENIS_PEMBAYARAN + '\',\'' + full.CURRENCY + '\',\'' + full.TOTAL_TAGIHAN + '\')"><i class="fa fa-arrows-alt"></i></button>' +
                ...
                },
                {
                    ...
                    "mRender": function (data, type, full) {
                        ...
                        var ret_value = '';
                        var role = newRoleUser[0];
                        if (role == "ROLE_MS_LIKUIDITAS" || role == "ROLE_DM_LIKUIDITAS") {
                            return ""
                        } else {
                            if (full.STATUS_TRACKING == "INPUT DATA") {
                                ...
                                if(role == "ROLE_MANAGER_INVESTMENT_APLN" || newRoleUser[0] == "ROLE_ADMIN" || role == "ROLE_MANAGER_INVESTMENT_SLPMN"){
                                ...
                                if(role == "ROLE_MANAGER_PAYMENT"|| role == "ROLE_ADMIN"){
                                ...
                                if(role == "ROLE_VICE_PRESIDENT_OPERATION" || role == "ROLE_ADMIN"){
                            ...
                            /*else if (full.STATUS_TRACKING == "VERIFIED BY VP TREASURY OPERATION"){
                                if(role == "ROLE_ADMIN" || role == "ROLE_EXECUTIVE_VICE_PRESIDENT"){
                                    value = '{"10":"'+full.ID_VALAS+'","jenisPembayaran" : "'+full.ID_JENIS_PEMBAYARAN+'", "currency":"'+full.CURRENCY+'", "total":"'+full.TOTAL_TAGIHAN+'"}';
                                }if(role.includes("KASIR") && full.EQ_IDR <= 35000000000){
                            ...
                            /*else if (full.STATUS_TRACKING == "VERIFIED BY VP TREASURY INVESTMENT"){
                                ...
                                if(role.includes("KASIR") || role == "ROLE_ADMIN"){
                                    value = '{"7":"'+full.ID_VALAS+'","jenisPembayaran" : "'+full.ID_JENIS_PEMBAYARAN+'", "currency":"'+full.CURRENCY+'", "total":"'+full.TOTAL_TAGIHAN+'"}';
                                }else {
                                    value = '{"x":"'+full.ID_VALAS+'","jenisPembayaran" : "'+full.ID_JENIS_PEMBAYARAN+'", "currency":"'+full.CURRENCY+'", "total":"'+full.TOTAL_TAGIHAN+'"}';
                                }
...
function initMultipleButton(role, jenis){
    ...
    $('.dataTables_filter').each(function () {
        ...
        if(role != "ROLE_OSS" && !role.includes("DIVKEU")){
            ...
            if(jenis == "1"){
                html = html + '<button class="btn-edit-data btn-sm btn-info" id="btn-verified" style="margin-left: 10px" type="button"  title="Edit Data" onclick="openMultipleEditForm()"><i class="fas fa-edit"></i></button>';
            }
        }
        ...
        html = html + '<button class="btn-delete btn-danger btn-sm" id="btn-verified" style="margin-left: 10px" type="button" title="Delete Data" onclick="multipleDelete()"><i class="fas fa-trash"></i></button></span>';
        $(this).append(html);
    });
}
st
