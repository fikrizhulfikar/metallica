...
function getAllData() {
    $.ajax({
        url: baseUrl + "api_operator/rekap_invoice_belum/get_rekap_invoice_employee",
        dataType: 'JSON',
        type: "GET",
        data: {
            pStatusValas: "0",
...
function initDataTable(pTglAwal, pTglAkhir, pBank, pCurrency, pCaraBayar, statusTracking) {
            ...
            table_rekapitulasi = $('#table-rekapitulasi').DataTable({
                    ...
                    "ajax":
                        {
                            "url":
                                baseUrl + "api_operator/rekap_invoice_belum/get_rekap_invoice_employee",
                            "type":
                                "GET",
                            "dataType":
                                "json",
...
function exportXlsCms() {
    if(invoiceCheckedArray.length <= 0){
        ...
        if(restitusiIsSame(invoiceCheckedArray) === true){
            ...
            if (bank == "MANDIRI" && metode == "GIRO" || metode == "ONLINETRANSFER"){
                cetakXlsCmsMandiri();
            } else if (bank == "BRI" && metode == "GIRO" || metode == "ONLINETRANSFER"){
                cetakXlsCmsBri();
            } else if (bank == "BNI" && metode == "GIRO" || metode == "ONLINETRANSFER"){
                cetakXlsCmsBni();
            } else {
