...
function getTotalTagihanValas() {
    $.ajax({
        url: baseUrl + "/api_operator/pembelian_valas_trx/get_total_tagihan",
        type: "GET",
        data: {
    ...
    if (checkedArray.length <= 0){
        ...
        Swal.fire({
            ...
            if (result.value){
                showLoadingCss()
                $.ajax({
                    url: baseUrl + "api_operator/pembelian_valas_trx/multiple_delete_head",
                    dataType: 'JSON',
                    type: "POST",
                    data: {
...
function AddToTable() {
    ...
    if (drcrind == "" || glaccount == "" || amt == "" || remarks == "" || cash_code == "") {
        ...
        let rowNode = pembelianValasDetail.row
            .add({
                "DOC_NO": docno,
    ...
    Swal.fire({
        ...
        if (response.value){
            if(checkedArray.length <=0){
                ...
                showLoadingCss();
                $.ajax({
                    url: baseUrl + "/api_operator/pembelian_valas_trx/multi_upd_lunas",
                    dataType: 'JSON',
                    type: "POST",
                    data: {
                        pData: JSON.stringify(checkedArray),
    ...
    tablePembelianValas = $('#table-rekapitulasi').DataTable({
            ...
            "aoColumnDefs": [
                ...
                },
                {
                    "aTargets": [11],
                    "mRender": function (data, type, full) {
                        return full.TGL_RENCANA_BYR;
                    }
                },
                {
                    "aTargets": [12],
                    "mRender": function (data, type, full) {
                        return full.JENIS_DOC;
                    }
                    ...
                    "mRender": function (data, type, full) {
                        ...
                        if (newRoleUser[0] == "ROLE_MS_LIKUIDITAS" || newRoleUser[0] == "ROLE_DM_LIKUIDITAS") {
                            ...
                            ret_value = ret_value +
                                ...
                                '<button style="width: 15px !important;" class="btn-delete-data btn-sm btn-danger" title="Delete" onclick="delete_data(\'' + full.ID_VALAS + '\')"><i class="fas fa-trash-alt"></i></button>' +
                                '</div>';
                        }else {
                            ...
                            if (full.STATUS_TRACKING == "INPUT DATA") {
                                ...
                                if(newRoleUser[0] == "ROLE_ADMIN" && full.TOTAL_TAGIHAN !== "-"){
                                    ...
                                    ret_value = ret_value +
                                        ...
                                        '<button style="width: 15px !important; margin-right: 5px !important;" class="btn btn-sm btn-info" title="Edit Data" onclick="edit_data(\'' +full.ID_METALLICA+'\')"><i class="far fa-edit"></i></button>'+
                                        '<button style="width: 15px !important; margin-right: 5px !important;" class="btn btn-sm btn-warning" title="Verified MAKER" onclick="update_status(\'' +full.ID_METALLICA+'\',\''+1+'\',\'' +full.AMOUNT_DEBIT+'\',\'' +full.AMOUNT_CREDIT+'\')"><i class="fa fa-arrows-alt"></i></button>'+
                                        '<button style="width: 15px !important;" class="btn btn-sm btn-danger" title="Hapus" onclick="deleteHead(\'' + full.ID_METALLICA + '\')"><i class="fas fa-trash-alt"></i></button>'+
                                        '</div>';
                                }
                                if(newRoleUser[0] == "ROLE_JA_CASH" && full.TOTAL_TAGIHAN !== "-"){
                                    ret_value = ret_value +
                                        '<button style="width: 15px !important; margin-right: 5px !important;" class="btn btn-sm btn-success" title="Detail" onclick="getDetails(\'' +full.ID_METALLICA+'\',\''+full.DOCUMENT_NUMBER+'\',\''+full.BUSINESS_AREA+'\',\''+full.COMPANY_CODE+'\',\''+full.REFERENCE+'\',\''+full.PMT_PROPOSAL_ID+'\',\''+full.POSTING_DATE+'\',\''+full.FISC_YEAR+'\',\''+full.CURRENCY+'\',\''+full.EXCHANGE_RATE+'\',\''+full.STATUS_TRACKING+'\')"><i class="fa fa-info-circle"></i></button>'+
                                        '<button style="width: 15px !important; margin-right: 5px !important;" class="btn btn-sm btn-info" title="Edit Data" onclick="edit_data(\'' +full.ID_METALLICA+'\')"><i class="far fa-edit"></i></button>'+
                                        '<button style="width: 15px !important; margin-right: 5px !important;" class="btn btn-warning" title="Verified MAKER" onclick="update_status(\'' +full.ID_METALLICA+'\',\''+1+'\',\'' +full.AMOUNT_DEBIT+'\',\'' +full.AMOUNT_CREDIT+'\')"><i class="fa fa-arrows-alt"></i></button>'+
                                        '<button style="width: 15px !important;" class="btn btn-sm btn-danger" title="Hapus" onclick="deleteHead(\'' + full.ID_METALLICA + '\')"><i class="fas fa-trash-alt"></i></button>'+
                                        '</div>';
                                }
                                ...
                                if(newRoleUser[0] == "ROLE_ADMIN"){
                                    ret_value = ret_value +
                                        ...
                                        '<button type="button" style="width: 15px !important; margin-right: 5px !important;" class="btn btn-edit-data btn-sm btn-warning" title="Verified Checker" onclick="update_status(\'' +full.ID_METALLICA+'\',\''+2+'\',\'' +full.AMOUNT_DEBIT+'\',\'' +full.AMOUNT_CREDIT+'\')"><i class="fa fa-arrows-alt"></i></button>'+
                                        '<button type="button" style="width: 15px !important;" class="btn btn btn-sm btn-elementary" title="Reverse Checker" onclick="reverse_status(\'' +full.ID_METALLICA+'\',\''+1+'\')"><i class="fa fa-arrow-left"></i></button>'+
                                        '</div>';
                                }
                                if(newRoleUser[0] == "ROLE_MSB_CENTRALIZED_RECEIPT"){
                                    ret_value = ret_value +
                                        '<button type="button" style="width: 15px !important; margin-right: 5px !important;" class="btn btn-edit-data btn-sm btn-success" title="Detail" onclick="getDetails(\'' +full.ID_METALLICA+'\',\''+full.DOCUMENT_NUMBER+'\',\''+full.BUSINESS_AREA+'\',\''+full.COMPANY_CODE+'\',\''+full.REFERENCE+'\',\''+full.PMT_PROPOSAL_ID+'\',\''+full.POSTING_DATE+'\',\''+full.FISC_YEAR+'\',\''+full.CURRENCY+'\',\''+full.EXCHANGE_RATE+'\',\''+full.STATUS_TRACKING+'\',\''+full.SUMBER_DANA+'\')"><i class="fa fa-info-circle"></i></button>'+
                                        '<button type="button" style="width: 15px !important; margin-right: 5px !important;" class="btn btn-edit-data btn-sm btn-warning" title="Verified Checker" onclick="update_status(\'' +full.ID_METALLICA+'\',\''+2+'\',\'' +full.AMOUNT_DEBIT+'\',\'' +full.AMOUNT_CREDIT+'\')"><i class="fa fa-arrows-alt"></i></button>'+
                                        '<button type="button" style="width: 15px !important;" class="btn btn-reverse-data btn-sm btn-elementary" title="Reverse Checker" onclick="reverse_status(\'' +full.ID_METALLICA+'\',\''+1+'\')"><i class="fa fa-arrow-left"></i></button>'+
                                        '</div>';
                                }
                            }
                                ...
                                if(newRoleUser[0] == "ROLE_ADMIN"){
                                    ret_value = ret_value +
                                        ...
                                        '<button type="button" style="width: 15px !important;" class= "btn btn btn-sm btn-elementary" title="Reverse APPROVER" onclick="reverse_status(\'' +full.ID_METALLICA+'\',\''+2+'\')"><i class="fa fa-arrow-left"></i></button>'+
                                        '</div>';
                                }
                                if(newRoleUser[0] == "ROLE_MSB_PAYMENT_EXPENDITURE"){
                                    ret_value = ret_value +
                                        '<button type="button" style="width: 15px !important; margin-right: 5px !important;" class="btn btn-sm btn-success" title="Detail" onclick="getDetails(\'' +full.ID_METALLICA+'\',\''+full.DOCUMENT_NUMBER+'\',\''+full.BUSINESS_AREA+'\',\''+full.COMPANY_CODE+'\',\''+full.REFERENCE+'\',\''+full.PMT_PROPOSAL_ID+'\',\''+full.POSTING_DATE+'\',\''+full.FISC_YEAR+'\',\''+full.CURRENCY+'\',\''+full.EXCHANGE_RATE+'\',\''+full.STATUS_TRACKING+'\',\''+full.SUMBER_DANA+'\')"><i class="fa fa-info-circle"></i></button>'+
                                        '<button type="button" style="width: 15px !important; margin-right: 5px !important;" class="btn btn-sm btn-warning" title="Verified APPROVER" onclick="update_status(\'' +full.ID_METALLICA+'\',\''+3+'\',\'' +full.AMOUNT_DEBIT+'\',\'' +full.AMOUNT_CREDIT+'\')"><i class="fa fa-arrows-alt"></i></button>'+
                                        '<button type="button" style="width: 15px !important;" class= "btn btn-reverse-data btn-sm btn-elementary" title="Reverse APPROVER" onclick="reverse_status(\'' +full.ID_METALLICA+'\',\''+2+'\')"><i class="fa fa-arrow-left"></i></button>'+
                                        '</div>';
                                }
                                ...
                                if(newRoleUser[0] == "ROLE_VP_LIQUIDITY_AND_RECEIPT"){
                                    ret_value = ret_value +
                                        ...
                                        '<button type="button" style="width: 15px !important;" class="btn btn-reverse-data btn-sm btn-elementary" title="Reverse Checker" onclick="reverse_status(\'' +full.ID_METALLICA+'\',\''+2+'\')"><i class="fa fa-arrow-left"></i></button>'+
                                        '</div>';
                                }
                                ...
                                if(newRoleUser[0] == "ROLE_VP_INVESTMENT_EXPENDITURE"){
                                    ret_value = ret_value +
                                        '<button type="button" style="width: 15px !important; margin-right: 5px !important;" class="btn btn-sm btn-success" title="Detail" onclick="getDetails(\'' +full.ID_METALLICA+'\',\''+full.DOCUMENT_NUMBER+'\',\''+full.BUSINESS_AREA+'\',\''+full.COMPANY_CODE+'\',\''+full.REFERENCE+'\',\''+full.PMT_PROPOSAL_ID+'\',\''+full.POSTING_DATE+'\',\''+full.FISC_YEAR+'\',\''+full.CURRENCY+'\',\''+full.EXCHANGE_RATE+'\',\''+full.STATUS_TRACKING+'\',\''+full.SUMBER_DANA+'\')"><i class="fa fa-info-circle"></i></button>'+
                                        '<button type="button" style="width: 15px !important; margin-right: 5px !important;" class="btn btn-sm btn-warning" title="Verified Checker" onclick="update_status(\'' +full.ID_METALLICA+'\',\''+3+'\',\'' +full.AMOUNT_DEBIT+'\',\'' +full.AMOUNT_KREDIT+'\')"><i class="fa fa-arrows-alt"></i></button>'+
                                        '<button type="button" style="width: 15px !important;" class="btn btn-reverse-data btn-sm btn-elementary" title="Reverse Checker" onclick="reverse_status(\'' +full.ID_METALLICA+'\',\''+2+'\')"><i class="fa fa-arrow-left"></i></button>'+
                                        '</div>';
                                }
                                ...
                                if(newRoleUser[0] == "ROLE_VP_LIQUIDITY_AND_RECEIPT"){
                                    ret_value = ret_value +
                                        '<button type="button" style="width: 15px !important; margin-right: 5px !important;" class="btn btn-sm btn-success" title="Detail" onclick="getDetails(\'' +full.ID_METALLICA+'\',\''+full.DOCUMENT_NUMBER+'\',\''+full.BUSINESS_AREA+'\',\''+full.COMPANY_CODE+'\',\''+full.REFERENCE+'\',\''+full.PMT_PROPOSAL_ID+'\',\''+full.POSTING_DATE+'\',\''+full.FISC_YEAR+'\',\''+full.CURRENCY+'\',\''+full.EXCHANGE_RATE+'\',\''+full.STATUS_TRACKING+'\',\''+full.SUMBER_DANA+'\')"><i class="fa fa-info-circle"></i></button>'+
                                        '<button type="button" style="width: 15px !important; margin-right: 5px !important;" class= "btn btn-sm btn-elementary" title="Reverse APPROVER" onclick="reverse_status(\'' +full.ID_METALLICA+'\',\''+3+'\')"><i class="fa fa-arrow-left"></i></button>'+
                                        '<button type="button" style="width: 15px !important;" class="btn btn-sm btn-primary" title="Pelunasan" onclick="updateLunas(\'' +full.ID_METALLICA+'\',\''+jenis+'\')"><i class="fas fa-credit-card"></i></button>'+
                                        '</div>';
                                }
                                if(newRoleUser[0] == "ROLE_ADMIN"){
                                    ret_value = ret_value +
                                ...
                                if(newRoleUser[0] == "ROLE_VP_INVESTMENT_EXPENDITURE"){
                                    ret_value = ret_value +
                                        '<buttonty type="button" style="width: 15px !important; margin-right: 5px !important;" class="btn btn-sm btn-success" title="Detail" onclick="getDetails(\'' +full.ID_METALLICA+'\',\''+full.DOCUMENT_NUMBER+'\',\''+full.BUSINESS_AREA+'\',\''+full.COMPANY_CODE+'\',\''+full.REFERENCE+'\',\''+full.PMT_PROPOSAL_ID+'\',\''+full.POSTING_DATE+'\',\''+full.FISC_YEAR+'\',\''+full.CURRENCY+'\',\''+full.EXCHANGE_RATE+'\',\''+full.STATUS_TRACKING+'\',\''+full.SUMBER_DANA+'\')"><i class="fa fa-info-circle"></i></buttonty>'+
                                        '<buttonty type="button" style="width: 15px !important; margin-right: 5px !important;" class= "btdata btn-sm btn-elementary" title="Reverse APPROVER" onclick="reverse_status(\'' +full.ID_METALLICA+'\',\''+3+'\')"><i class="fa fa-arrow-left"></i></buttonty>'+
                                        '<buttonty type="button" style="width: 15px !important;" class="btn btn-sm btn-primary" title="Pelunasan" onclick="updateLunas(\'' +full.ID_METALLICA+'\',\''+jenis+'\')"><i class="fas fa-credit-card"></i></buttonty>'+
                                        '</div>';
                                }
                                 if(newRoleUser[0] == "ROLE_VP_BUSINESS_MANAGEMENT"){
                                    ret_value = ret_value +
                                        '<buttonty type="button" style="width: 15px !important; margin-right: 5px !important;" class="btn btn-sm btn-success" title="Detail" onclick="getDetails(\'' +full.ID_METALLICA+'\',\''+full.DOCUMENT_NUMBER+'\',\''+full.BUSINESS_AREA+'\',\''+full.COMPANY_CODE+'\',\''+full.REFERENCE+'\',\''+full.PMT_PROPOSAL_ID+'\',\''+full.POSTING_DATE+'\',\''+full.FISC_YEAR+'\',\''+full.CURRENCY+'\',\''+full.EXCHANGE_RATE+'\',\''+full.STATUS_TRACKING+'\',\''+full.SUMBER_DANA+'\')"><i class="fa fa-info-circle"></i></buttonty>'+
                                        '<buttonty type="button" style="width: 15px !important; margin-right: 5px !important;" class= "btdata btn-sm btn-elementary" title="Reverse APPROVER" onclick="reverse_status(\'' +full.ID_METALLICA+'\',\''+3+'\')"><i class="fa fa-arrow-left"></i></buttonty>'+
                                        '<buttonty type="button" style="width: 15px !important;" class="btn-edit-data btn-sm btn-primary" title="Pelunasan" onclick="updateLunas(\'' +full.ID_METALLICA+'\',\''+jenis+'\')"><i class="fas fa-credit-card"></i></buttonty>'+
                                        '</div>';
                                }
                                ...
                                if(newRoleUser[0] == "ROLE_MSB_LOCAL_CURRENCY_LIQUIDITY"){
                                    ret_value = ret_value +
                                        '<buttonty type="button" style="width: 15px !important; margin-right: 5px !important;" class="btn btn-sm btn-success" title="Detail" onclick="getDetails(\'' +full.ID_METALLICA+'\',\''+full.DOCUMENT_NUMBER+'\',\''+full.BUSINESS_AREA+'\',\''+full.COMPANY_CODE+'\',\''+full.REFERENCE+'\',\''+full.PMT_PROPOSAL_ID+'\',\''+full.POSTING_DATE+'\',\''+full.FISC_YEAR+'\',\''+full.CURRENCY+'\',\''+full.EXCHANGE_RATE+'\',\''+full.STATUS_TRACKING+'\',\''+full.SUMBER_DANA+'\')"><i class="fa fa-info-circle"></i></buttonty>'+
                                        '<buttonty type="button" style="width: 15px !important; margin-right: 5px !important;" class= "btdata btn-sm btn-default" title="Reverse APPROVER" onclick="reverse_status(\'' +full.ID_METALLICA+'\',\''+3+'\')"><i class="fa fa-arrow-left"></i></buttonty>'+
                                        '<buttonty type="button" style="width: 15px !important;" class="btn-edit-data btn-sm btn-primary" title="Pelunasan" onclick="updateLunas(\'' +full.ID_METALLICA+'\',\''+jenis+'\')"><i class="fas fa-credit-card"></i></buttonty>'+
                                        '</div>';
                                }
                                 if(newRoleUser[0] == "ROLE_VP_INVESTMENT_EXPENDITURE"){
                                ...
                                ret_value =
                                    '<div class="btn-group">' +
                                    '<button type="button" style="width: 15px !important; margin-right: 5px !important;" class="btn btn-sm btn-success" title="Detail" onclick="getDetails(\''+full.DOC_NO+'\',\''+full.COMP_CODE+'\',\''+full.FISC_YEAR+'\')"><i class="fa fa-info-circle"></i></button>'+
                                '</div>';
                            }
                        }
                        return ret_value;
            ...
            "ajax":
                {
                    "url":
                        baseUrl + "api_invoice_pilot/hrap_invoice/get_list_nonvendor_head",
                    "type":
                        "GET",
                    "dataType":
                        "json",
...
//         var html = '';
        ...
        if(newRoleUser[0] == "ROLE_FCL_SETTLEMENT") {
            html = html + '<button class="btn btn-danger btn-sm" id="btn-verified" style="margin-left: 10px !important; width: 32px !important;" type="button" title="Delete Data" onclick="multipleDelete()"><i class="fas fa-trash-alt"></i></button>';
        }
        if(newRoleUser[0] == "ROLE_VP_LIQUIDITY_AND_RECEIPT" || newRoleUser[0] == "ROLE_VP_INVESTMENT_EXPENDITURE"
            ...
            html = html +'<button class="btn btn-sm btn-primary" style="margin-left: 10px !important; width: 32px !important;" type="button" title="Pelunasan" onclick="multi_upd_lunas()"><i class="fas fa-credit-card"></i></button>';
        }
        if(newRoleUser[0] === "ROLE_ADMIN"){
            html = html + '<button class="btn btn-danger btn-sm" id="btn-verified" style="margin-left: 10px !important; width: 32px !important;" type="button" title="Delete Data" onclick="multipleDelete()"><i class="fas fa-trash-alt"></i></button>'+
            '<button class="btn btn-sm btn-primary" style="margin-left: 10px !important; width: 32px !important;" type="button" title="Pelunasan" onclick="multi_upd_lunas()"><i class="fas fa-credit-card"></i></button>';
        }
        $(this).append(html);
    });
...
function submitChild() {
    let table_data = pembelianValasDetail.data();
    dataList = [];
...
function getDetails(doc_no, comp_code, fisc_year) {
    $(".list-data").hide();
    $(".detail-data").show();
    $("#filter").hide();
    $("#btn-add-rekap").hide();
    ...
    pembelianValasDetail = $("#table-main-detail").DataTable({
        "ajax" : {
            "url" : baseUrl + "api_invoice_pilot/hrap_invoice/get_list_nonvendor_item",
            "data" : {
                "pDocNo": doc_no,
                "pFiscalYear" : fisc_year,
                "pCompCode" : comp_code
            },
            "type": "GET",
            "dataType": "json",
        ...
        "columns" : [
            ...
            {"data" : "ITEM_TEXT"},
            {
               ...
               "render" : function (data2, type, row ) {
                   let number = parseInt(data2.AMOUNT);
            ...
            {"data" : "JENIS_DOK"},
            {
                "data" : null,
                "render" : (data) => {
                    return '<td align="center"> <button class="btn btn-sm btn-danger" onclick="deletedb()"><i class="fa fa-trash"></i></button></td>';
                }
            }
        ],
    ...
    Valas = {
        ...
        valasDetails: null //ini harus sama dengan nama variable di class nya
    }
...
function edit_data (idMetallica){
    showLoadingCss();
    $.ajax({
        url : baseUrl + "api_operator/pembelian_valas_trx/get_valas_head_byid",
        dataType : "JSON",
        type : "GET",
        data : {
            pIdMetallica : idMetallica,
...
function ins_data() {
    let idValas = $("#pIdMetallica").val();
    showLoadingCss();
    (idValas === undefined || idValas === "") ? idValas = null : idValas=idValas;
