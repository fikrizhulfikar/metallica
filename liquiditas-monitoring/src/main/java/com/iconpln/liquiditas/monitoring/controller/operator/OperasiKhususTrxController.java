...
//                notificationUtil.notifyMessage(notification);
    ...
    @RequestMapping(value = "/rekap_operasi_khusus_trx_lunas", method = RequestMethod.GET)
    public Map rekapOperasiKhususTrxLunas(
            @RequestParam(value = "draw", defaultValue = "0") int draw,
            @RequestParam(value = "start", defaultValue = "0") int start,
            @RequestParam(value = "length", defaultValue = "10") int length,
            @RequestParam(value = "columns[0][data]", defaultValue = "") String firstColumn,
        ...
        List<Map<String, Object>> list = new ArrayList<>();
        try {
            list = operasiKhususTrxService.getRekapOperasiKhususTrxLunas(((start / length) + 1), length, pTglAwal, pTglAkhir, pCurrency, WebUtils.getUsernameLogin(), sortBy, sortDir, pStatus, pStatusTracking, pSearch);
        } catch (Exception e) {
            e.printStackTrace();
        }
