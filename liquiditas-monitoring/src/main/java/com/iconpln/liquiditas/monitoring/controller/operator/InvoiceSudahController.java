 ...
 */
@RestController
@RequestMapping("/api_operator/rekap_invoice_sudah")
public class InvoiceSudahController {

    @Autowired
    InvoiceSudahService invoiceSudahService;
    ...
    public Map listRekapDataBelum(
        ...
        try {
            list = invoiceSudahService.getListPembayaranSudah(((start / length) + 1), length, pTglAwal, pTglAkhir, pBank, pCurrency, pCaraBayar, WebUtils.getUsernameLogin(), sortBy, sortDir, pStatus, pStatusTracking, pSearch);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public List getDataInvoiceBy(
        ...
        try {
            return invoiceSudahService.getDataInvoiceBy(pCompCode, pNoDoc, pFiscYear, pLineItem );
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
...
//                          @RequestParam("unit") Integer unit,
        ...
        Map data = new HashMap();
        try {
            String result = invoiceSudahService.saveColumn(WebUtils.getUsernameLogin(),
                    nomor,
                    compCode,
                    docNo,
                    fiscYear,
...
//        AppUtils.getLogger(this).info("pMetodePembayaran edit data: {}", pMetodePembayaran);
        try {
            Map<String, Object> res = invoiceSudahService.updatePembayaran(pCompCode, pDocNo, pFiscYear, pLineItem, pKet, pBankPembayar, pKeterangan, pTglRencanaBayar,pSumberDana,
...
//        AppUtils.getLogger(this).info("pMetodePembayaran edit data: {}", pMetodePembayaran);
        try {
            Map<String, Object> res = invoiceSudahService.updateLunas(pCompCode, pDocNo, pFiscYear, pLineItem, pJenisTransaksi, WebUtils.getUsernameLogin(), pStatus);
            if (((BigDecimal) res.get("return")).equals(BigDecimal.ONE)) {
...
//        AppUtils.getLogger(this).info("pMetodePembayaran edit data: {}", pMetodePembayaran);
        try {
            Map<String, Object> res = invoiceSudahService.updateStatus(pCompCode, pDocNo, pFiscYear, pLineItem, pKet, pStatusTracking,
...
//        AppUtils.getLogger(this).info("pMetodePembayaran edit data: {}", pMetodePembayaran);
        try {
            Map<String, Object> res = invoiceSudahService.reverseStatus(pCompCode, pDocNo, pFiscYear, pLineItem, pKet, pStatusTracking);
            if (((BigDecimal) res.get("return")).equals(BigDecimal.ONE)) {
    ...
    public List<Map<String,Object>> getBankPembayar() {
        try {
            return invoiceSudahService.getBankPembayar();
        } catch (Exception e) {
    ...
    public List<Map<String,Object>> getMetodeBayar() {
        try {
            return invoiceSudahService.getMetodeBayar();
        } catch (Exception e) {
            AppUtils.getLogger(this).debug(e.getMessage());
            return null;
        }
    ...
    public List<Map<String,Object>> getBankAccount(@RequestParam(value = "pBankPembayar", defaultValue = "") String pBankPembayar) {
        ...
        try {
            return invoiceSudahService.getBankAccount(pBankPembayar);
        } catch (Exception e) {
            AppUtils.getLogger(this).debug(e.getMessage());
            return null;
        }
    ...
    public List<Map<String,Object>> getAccountName(@RequestParam(value = "pBankAccount", defaultValue = "") String pBankAccount) {
        ...
        try {
            return invoiceSudahService.getAccountName(pBankAccount);
        } catch (Exception e) {
            AppUtils.getLogger(this).debug(e.getMessage());
            return null;
        }
    ...
    public List<Map<String,Object>> getSaldo(@RequestParam(value = "pBankAccount", defaultValue = "") String pBankAccount) {
        ...
        try {
            return invoiceSudahService.getSaldo(pBankAccount);
        } catch (Exception e) {
            AppUtils.getLogger(this).debug(e.getMessage());
            return null;
        }
    ...
    public Map<String, Object> insRekapReject(
        ...
        try {
            return invoiceSudahService.insRejectLaporan(pCompCode, pDocNo, pFiscYear, pLineItem, pKet);
    ...
    public String getBallance(
        ...
        return invoiceSudahService.getBallance(in_bank, in_source, in_beneficiary);
    }
    ...
    public String getBallance2(
        ...
        try {
            String res = invoiceSudahService.get2(pBank, pSource, pBeneficiary);
    ...
    public String inquiry(
        ...
        try {
            String res = invoiceSudahService.doInquery(pBank, pSource, pBeneficiaryAccount);
            System.out.println("TEST");
            System.out.println(res);
            // if (((BigDecimal) res.get("return")).equals(BigDecimal.ONE)) {
    ...
    public String inquiry(
        ...
        try {
            String res = invoiceSudahService.payment(pMetodeBayar,pBank,pRefNum, pSource, pBeneficiaryAccount,pCurrency,
    ...
    public String export(
        ...
        try {
            ...
            String namaFile = "rekap_invoice_verified.xls";

            ServletOutputStream os = response.getOutputStream();
            response.setContentType("application/vnd.ms-excel");
            response.setHeader("Content-Disposition", "attachment; filename=\"" + namaFile + "\"");

            List<Map<String, Object>> listData = invoiceSudahService.getAllPembayaran(WebUtils.getUsernameLogin(), tglAwal.replaceAll("-", "/"), tglAkhir.replaceAll("-", "/"), pCurr, statusTracking, pBank, caraBayar, status);
            ...
            InputStream streamTemplate = resourceLoader.getResource("classpath:/templates/report/rekap_invoice_verified.xls").getInputStream();
    ...
    public String getTotalTagihan(@RequestParam(value = "tgl_awal", defaultValue = "") String tglAwal,
        ...
        BigDecimal result =  invoiceSudahService.getTotalTagihan(tglAwal, tglAkhir, currency, caraBayar, bank, WebUtils.getUsernameLogin(), search);
